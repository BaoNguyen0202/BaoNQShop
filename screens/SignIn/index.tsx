...
const login = async (navigation: any) => {
  ...
  var auth = new UserInfo();
  auth.email = 'admin@medusa-test.com';
  auth.password = 'supersecret';
  auth.token = TOKEN;
  // try {
  //   await fetch(API_STORE_URL + 'auth', {
  //     method: 'POST',
  //     headers: {
  ...
  //     AsyncStorage.setItem('UserInfo', JSON.stringify(auth));
  //     AsyncStorage.setItem('CartId', '');
  //     AsyncStorage.setItem('Email', auth.email);
  //     RNRestart.Restart();
...
export function SignInForm({props}: any) {
  ...
  const [appId, setAppId] = React.useState<number>(133391); //80810 //129743
            ...
            <VStack space="3">
              <VStack space={{base: '7', md: '4'}}>
                <FloatingLabelInput
                  ...
                  borderRadius="4"
                  defaultValue="admin@medusa-test.com"
